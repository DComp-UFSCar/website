<?php
  ...
  class Course {
    ...
    public function loadById($id) {
      $db = new Db();
      $id = $db->quote($id);
      $materia = $db->selectOne("SELECT * FROM materia WHERE cod = ".$id);

      $this->id = $id;
      $this->codigo   = $materia["cod2"];
      $this->abrev    = $materia["codDisciplina"];
      $this->nome     = $materia["nome"];
      $this->ementa   = $materia["ementa"];
      $this->objetivo = $materia["objetivo"];
      $this->credTeorico = $materia["nCreditoTeorico"];
      $this->credPratico = $materia["nCreditopratico"];
    ...
    public function loadOfertas() {
      ...
      for ($i = 0; $i < count($this->ofertas); $i++) {
        ...
        $this->ofertas[$i]['professor'] = $db->selectOne("SELECT cod, nome, pagPessoal FROM professor WHERE ativo = 1 AND cod = ".$this->ofertas[$i]['codProf']);
      }
    }
  }
?>