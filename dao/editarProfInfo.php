...
//    addMateria
   ...
   $mysql = new conexao;
    try {
     ...
     $codigo = $_POST["cod"];     
     $resultBD = $mysql->sql_query("SELECT `cod`, `pagPessoal`, `areaAtuacao`, `nome`, `ativo`, `foto` FROM `professor` WHERE cod =".$_SESSION['cod']);
     $prof = mysql_fetch_array($resultBD, MYSQL_ASSOC);
//     print_r($prof);exit();
     
     $resultBD = $mysql->sql_query("SELECT codAdm, codProf FROM professoradm WHERE codProf =".$_SESSION['cod']);
