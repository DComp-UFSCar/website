...
function getColor($num) {
    ...
    $mysql = new conexao;
    try {
    $listaBD = $mysql->sql_query("select paginamateria.codMat, paginamateria.codPag, pagina.cod, pagina.pagina, pagina.titulo, materia.cod2, ofertamateria.codProf 
                                    FROM paginamateria, pagina, materia, ofertamateria
                                    WHERE paginamateria.ativo = 1 AND  pagina.cod = paginamateria.codPag AND paginamateria.codMat = materia.cod AND materia.cod = ofertamateria.codMat AND ofertamateria.codProf = ".$_SESSION['cod']." ORDER BY paginamateria.codPag DESC");
